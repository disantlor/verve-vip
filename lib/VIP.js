...
async function completeDraftOrder(draft_order_id) {
    ...
    if (! charge || ! charge.id) {
        ...
        throw new Error("Something went wrong capturing payment")
    }
    ...
    try {
        ...
        try {
            let cancelled_payment = await Stripe.paymentIntents.cancel(charge.id)
    ...
    try {
        console.log("About to complete payment", charge.id)
